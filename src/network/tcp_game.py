...
async def game_session(
    ...
    try:
        while True:
            ui.set_turn(my_turn)
            if my_turn:
                result = await _my_attack(writer, reader, game_id, ui, player_id, opponent_id, my_attacks)
            else:
...
async def _my_attack(writer, reader, game_id: str, ui: GameUI, player_id: str, opponent_id: str | None, attacks: set[Cell]) -> str | None:
